...
namespace WebAPI.EndpointMapping
{
    public static class ProductEndpoint
    {
        public static void MapProductEndpoints(this IEndpointRouteBuilder routes)
        {
            ...
            group.MapGet("/{id:int}", async (int id, IProductAppService svc)
                => (await svc.GetAsync(id)) is { } dto ? Results.Ok(dto) : Results.NotFound());

            group.MapPost("/", async (CreateProductCommand cmd,IValidator<CreateProductCommand> validator,IProductAppService svc) =>
            {
            ...
            group.MapPut("/{id:int}", async (int id, UpdateProductCommand cmd, IProductAppService svc)
                => await svc.UpdateAsync(id, cmd) ? Results.NoContent() : Results.NotFound()).AddEndpointFilter<ValidationFilter<UpdateProductCommand>>();

            group.MapDelete("/{id:int}", async (int id, IProductAppService svc)
